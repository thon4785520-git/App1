...
$expert = null;
if (!empty($_GET['id'])) {
    $expert = find_expert((int) $_GET['id']);
    if (!$expert || !expert_owner_can_edit($expert)) {
        flash('error', 'คุณไม่มีสิทธิ์แก้ไขรายการนี้');
        redirect('experts.php');
    }
}

$title = $expert ? 'แก้ไขโปรไฟล์ผู้เชี่ยวชาญ' : 'เพิ่มโปรไฟล์ผู้เชี่ยวชาญ';
$sections = [
    'work_experience' => ['title' => 'ข้อมูลการปฏิบัติงาน', 'fields' => ['organization' => 'หน่วยงาน', 'project_name' => 'โครงการ', 'role_title' => 'บทบาทหน้าที่', 'start_date' => 'เริ่ม', 'end_date' => 'สิ้นสุด', 'description' => 'รายละเอียด']],
    ...
    'training' => ['title' => 'ข้อมูลการพัฒนาตนเอง', 'fields' => ['course_name' => 'หลักสูตร', 'provider_name' => 'หน่วยงาน', 'certificate_name' => 'ใบประกาศ', 'certificate_file' => 'ไฟล์ประกาศ', 'start_date' => 'เริ่ม', 'end_date' => 'สิ้นสุด', 'description' => 'รายละเอียด']],
    'seminars' => ['title' => 'ประวัติการเข้าร่วมสัมมนา', 'fields' => ['seminar_name' => 'ชื่อสัมมนา', 'organizer_name' => 'ผู้จัด', 'joined_date' => 'วันที่เข้าร่วม', 'description' => 'รายละเอียด']],
    'awards' => ['title' => 'ข้อมูลอื่น ๆ / รางวัล', 'fields' => ['title' => 'ชื่อรางวัล', 'issuer_name' => 'หน่วยงาน', 'award_year' => 'ปี', 'description' => 'รายละเอียด']],
    'social_links' => ['title' => 'Social Links', 'fields' => ['platform_name' => 'แพลตฟอร์ม', 'link_url' => 'URL']],
];
require __DIR__ . '/../includes/header.php';
?>
<form method="post" enctype="multipart/form-data" action="<?= e(base_url('actions/save_expert.php')); ?>">
    <input type="hidden" name="csrf_token" value="<?= e(csrf_token()); ?>">
    <?php if ($expert): ?><input type="hidden" name="id" value="<?= (int) $expert['id']; ?>"><?php endif; ?>
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <div class="form-row">
                ...
                <div class="col-md-8 form-group"><label>ความเชี่ยวชาญพิเศษ</label><textarea class="form-control" rows="3" name="expertise_summary"><?= e($expert['expertise_summary'] ?? old('expertise_summary')); ?></textarea></div>
                ...
                <div class="col-md-8 form-group"><label>Skill tags</label><input class="form-control" id="skillInput" name="skills" value="<?= e($expert ? implode(', ', array_column($expert['skills'], 'name')) : old('skills')); ?>" placeholder="PHP, MySQL, Data Analytics"></div>
                <?php if (current_user()['role'] === 'admin' && $expert): ?>
    ...
    <?php endforeach; ?>
    <button class="btn btn-primary btn-lg mr-2 mb-2"><i class="fas fa-save mr-1"></i><?= $expert ? 'บันทึกการแก้ไข' : 'บันทึกโปรไฟล์'; ?></button>
</form>
<?php if ($expert && current_user()['role'] === 'admin'): ?>
    <form method="post" action="<?= e(base_url('actions/delete_expert.php')); ?>" class="mb-2" onsubmit="return confirm('ยืนยันการลบข้อมูลนี้?');">
        <input type="hidden" name="csrf_token" value="<?= e(csrf_token()); ?>">
        <input type="hidden" name="id" value="<?= (int) $expert['id']; ?>">
        <button class="btn btn-outline-danger btn-lg"><i class="fas fa-trash mr-1"></i>ลบโปรไฟล์</button>
    </form>
<?php endif; ?>
